...
contract ClearingServiceTest is Test {
    ...
    function test_depositInsuranceFund_revertsIfInvalidAmount() public {
        vm.startPrank(exchange);
    ...
    function test_withdrawInsuranceFundEmergency_revertsIfInvalidAmount() public {
        vm.startPrank(exchange);
    ...
    function test_insuranceCoverLost() public {
        vm.startPrank(exchange);
        ...
        clearingService.insuranceCoverLost(account, uint256(-loss), spotEngine, token);
        assertEq(spotEngine.getBalance(account, token), int256(0));
        assertEq(clearingService.getInsuranceFund(), fund - uint256(-loss));
    }

    function test_insuranceCoverLost_revertsWhenUnauthorized() public {
        vm.expectRevert(bytes(NOT_SEQUENCER));
        clearingService.insuranceCoverLost(account, 10, spotEngine, token);
    }

    function test_insuranceCoverLost_revertsIfSpotNotNegative() public {
        vm.startPrank(exchange);
        ...
        vm.expectRevert(bytes(SPOT_NOT_NEGATIVE));
        clearingService.insuranceCoverLost(account, balance, spotEngine, token);
    }

    function test_insuranceCoverLost_revertsIfInsufficientFund() public {
        vm.startPrank(exchange);
        ...
        vm.expectRevert(bytes(AMOUNT_EXCEEDS_FUND));
        clearingService.insuranceCoverLost(account, uint256(-loss), spotEngine, token);
    }
}
